...
def getsingledata_final(s, lungnae, f, rightflagset):
    # TODO - Change this
...
def getsingledata(s, lungname, f, rightflagset):
    # TODO - Change this
    sourcedir = 'Paper_KFoldData_Masterv2'
    if (s == 'lung'):
        if(rightflagset):
            lungdir = 'LungSeg_'+'Right'
        else:
...
def getdoubledata(s, lungname, f, rightflagset):
    ...
    #testdir = 'OutofBoxTest_Stu'
    #testdir_balanced = os.path.join(basedirnew, testdir)
    testdir = 'TestFinal_Fold' + str(f)
    testdir_balanced = os.path.join(basedir, testdir)
    valdir = 'ValFinal_Fold' + str(f)
    valdir_balanced = os.path.join(basedir, valdir)
        ...
        for f in allfolds:
            ...
            if(sampling=='under'):
                traindir_balanced, testdir_balanced, valdir_balanced, traindir_unbalanced, testdir_unbalanced, valdir_unbalanced = \
                    getsingledata_final(s, lungname, f, rightflagset)
            elif(sampling=='double'):
                traindir_balanced, testdir_balanced, valdir_balanced, traindir_unbalanced, testdir_unbalanced, valdir_unbalanced = \
                    getdoubledata(s, lungname, f, rightflagset)
            elif(sampling=='over'):
                traindir_balanced, testdir_balanced, valdir_balanced, traindir_unbalanced, testdir_unbalanced, valdir_unbalanced = \
            ...
            if (modelflag):
                if(modifiedflag==True):
                    ...
                    model, model_noft = getmodel_vggnet.build_model_doublelayer_modified(trainX_balanced, trainY_balanced,
                    ...
                    model, model_noft = getmodel_vggnet.build_model_doublelayer(trainX_balanced, trainY_balanced,
